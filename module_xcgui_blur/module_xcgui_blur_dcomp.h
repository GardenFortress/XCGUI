...
// 调用应缓存结果 (轻量, 但每次调要走 GetVersionEx + 模块加载查询).
//-----------------------------------------------------------------------------
bool IsSupported();
...
// Apply → owner-owned + WS_EX_APPWINDOW → 装 2 个 subclass → ShowWindow). 内部 acrylic
// HWND 存在静态全局, 通过 GetAcrylicHwnd() 取出.
//
...
//       module_xcgui.h.
// 其他参数同 Apply.
// 返回 acrylic HWND (= GetAcrylicHwnd()), 失败 NULL.
//
...
//-----------------------------------------------------------------------------
// 取 AttachAcrylicHost 创建的 acrylic HWND. 没建过 → NULL.
