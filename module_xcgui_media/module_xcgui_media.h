#ifndef  XCGUI_MEDIA_H
#define  XCGUI_MEDIA_H
//@模块名称  炫彩界面库-媒体(image+video)
//@版本  1.0.0
//@日期  2026-06-11
//@作者  未闻花名
//@QQ    936599025
//@依赖  module_xcgui_class.h
//@模块备注  CXImageEx(增强图片) + CXVideo(视频播放器) 合并模块.
//          共享 FFmpeg / 渲染 / 窗口几何内核; 公开 API 与别名保持不变.
//          旧 module_xcgui_image.h / module_xcgui_video.h 为兼容 shim.
//@模块信息结束
...
enum ximage_fit_mode_
{
	...
	//@别名 图片适配模式_等比裁切
	ximage_fit_cover    = (int)xmedia_fit_cover,      ///<等比缩放, 短边贴边, 长边裁切, 不留底色
	//@别名 图片适配模式_拉伸填满
	ximage_fit_stretch  = (int)xmedia_fit_stretch,    ///<不保持比例, 直接拉伸到元素客户区
	//@别名 图片适配模式_原始尺寸
	ximage_fit_original = (int)xmedia_fit_original,   ///<不缩放, 居中显示原图像素 (大于元素时裁切)
};
...
//@备注 设置背景色 (元素未被图片占满时填充用). 默认透明 (RGBA(0,0,0,0)). 支持 alpha.
//@别名  设置背景色()
...
private:
	...
	std::mutex           m_curFrameMutex;       // 保护 m_curBgra 与尺寸; OnPaint 用 shared_ptr 快照
	std::shared_ptr<const std::vector<uint8_t>> m_curBgra;  // size = m_curW * m_curH * 4
	...
	// ===== 渲染资源 =====
	_XMedia_Render_D2DBmpCache m_d2dCache;
	_XMedia_GdiDib             m_gdiOffscreen;
	...
	void OnExitSizeMoveImpl();
	static void OnSizeMoveExitThunk(void* user);
...
enum xvideo_fit_mode_
{
	...
	//@别名 视频适配模式_等比裁切
	xvideo_fit_cover    = (int)xmedia_fit_cover,      ///<等比缩放, 短边贴边, 长边裁切, 不留黑边
	//@别名 视频适配模式_拉伸填满
	xvideo_fit_stretch  = (int)xmedia_fit_stretch,    ///<不保持比例, 直接拉伸到元素客户区
	//@别名 视频适配模式_原始尺寸
	xvideo_fit_original = (int)xmedia_fit_original,   ///<不缩放, 居中显示原视频像素 (大于元素时裁切)
};
...
private:
	...
	// 默认黑底白字; 用户可在 Create() 前后调 SetControlBarColor / SetControlBarTextColor
	...
	std::shared_ptr<const std::vector<uint8_t>> m_curBgra;
	int    m_curW = 0, m_curH = 0;
	int    m_curPitch = 0;
	...
	// ===== 渲染资源 =====
	// D2D / GDI 离屏缓存 (L1 _XMedia_Render_* 管理).
	_XMedia_Render_D2DBmpCache m_d2dCache;
	_XMedia_GdiDib             m_gdiOffscreen;
	// DIB 内容是否需要重画. true = 必须重画才能 BitBlt 出新画面.
	bool                       m_gdiDibDirty = true;
	...
	// 错误派发 (供 worker 线程调). 统一走 _XMedia_FF_ErrToWide 转文案.
	void PostError(int code, const wchar_t* msg);
	void PostError(int code, const std::wstring& msg);
	// UI 线程 timer 里轮询 pending flag 并触发用户回调.
	void DispatchPendingCallbacks();
