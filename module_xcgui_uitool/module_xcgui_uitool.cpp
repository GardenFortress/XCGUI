...
constexpr int kCal_CornerRadius = kCornerRadius;
...
struct _XCal_Ctx
{
	...
	xcalendar_theme_ theme = xcalendar_theme_auto;
	...
	int contentW = 0;
	int contentH = 0;
	int contentOffX = kShadowMargin;
	int contentOffY = kShadowMargin;
...
BOOL _XCal_DetectDarkMode()
{
	return DetectSystemDarkMode();
}

void _XCal_ResolveTheme(xcalendar_theme_ theme, _XCal_ThemeColors* c)
{
	xcalendar_theme_ effective = theme;
	if (effective == xcalendar_theme_auto){
		effective = _XCal_DetectDarkMode() ? xcalendar_theme_dark : xcalendar_theme_light;
	}
	...
	if (effective == xcalendar_theme_light){
		c->wndBg            = kTheme_LightBg;
		c->text             = kTheme_LightText;
		c->mutedText        = WithAlpha(kTheme_LightText, 150);
		c->title            = RGBA(0x26, 0x71, 0xFC, 0xFF);
		c->weekText         = RGBA(0x6F, 0x6F, 0x6F, 0xFF);
		c->border           = WithAlpha(kTheme_LightText, 38);
		c->cellBg           = WithAlpha(kTheme_LightText, 14);
		c->cellHover        = WithAlpha(kTheme_LightText, 24);
		c->cellDisabledBg   = WithAlpha(kTheme_LightText, 8);
		c->cellDisabledText = WithAlpha(kTheme_LightText, 70);
		c->accent           = kTheme_LightText;
		c->accentSoft       = WithAlpha(kTheme_LightText, 32);
		c->accentText       = RGBA(0xFF, 0xFF, 0xFF, 0xFF);
		c->rangeText        = RGBA(0x17, 0x17, 0x17, 0xFF);
		c->inputBg          = kTheme_LightBg;
		c->inputBorder      = WithAlpha(kTheme_LightText, 58);
		c->buttonBg         = kTheme_LightBg;
		c->buttonHover      = WithAlpha(kTheme_LightText, 18);
		c->buttonBorder     = WithAlpha(kTheme_LightText, 58);
		c->cancelBg         = WithAlpha(kTheme_LightText, 14);
	...
	c->wndBg            = kTheme_DarkBg;
	c->text             = kTheme_DarkText;
	c->mutedText        = WithAlpha(kTheme_DarkText, 145);
	c->title            = RGBA(0x19, 0x63, 0xEF, 0xFF);
	c->weekText         = RGBA(0x7D, 0x7E, 0x7F, 0xFF);
	c->border           = WithAlpha(kTheme_DarkText, 40);
	c->cellBg           = WithAlpha(kTheme_DarkText, 18);
	c->cellHover        = WithAlpha(kTheme_DarkText, 32);
	c->cellDisabledBg   = WithAlpha(kTheme_DarkText, 10);
	c->cellDisabledText = WithAlpha(kTheme_DarkText, 68);
	c->accent           = kTheme_DarkText;
	c->accentSoft       = WithAlpha(kTheme_DarkText, 38);
	c->accentText       = RGBA(0xFF, 0xFF, 0xFF, 0xFF);
	c->rangeText        = RGBA(0xF5, 0xF5, 0xF5, 0xFF);
	c->inputBg          = WithAlpha(kTheme_DarkText, 10);
	c->inputBorder      = WithAlpha(kTheme_DarkText, 64);
	c->buttonBg         = WithAlpha(kTheme_DarkText, 16);
	c->buttonHover      = WithAlpha(kTheme_DarkText, 28);
	c->buttonBorder     = WithAlpha(kTheme_DarkText, 58);
	c->cancelBg         = WithAlpha(kTheme_DarkText, 12);
...
{
	wchar_t buf[64]{};
	swprintf_s(buf, _countof(buf), L"%04d.%02d.%02d -- %04d.%02d.%02d",
...
{
	if (!ctx) return TRUE;
	if (ctx->theme == xcalendar_theme_light) return TRUE;
	if (ctx->theme == xcalendar_theme_dark) return FALSE;
	return !_XCal_DetectDarkMode();
}
...
int CALLBACK _XCal_OnPaintWindow(HWINDOW hWnd, HDRAW hDraw, BOOL* pbHandled)
{
	...
	int innerW = cw - ctx->contentOffX - kShadowMargin;
	int innerH = ch - ctx->contentOffY - kShadowMargin;
	...
	for (int i = kShadowSteps; i >= 1; --i){
		...
		XDraw_SetLineWidthF(hDraw, i * kShadowMaxBlurFactor);
		XDraw_SetBrushColor(hDraw, ARGB(0, 0, 0, alpha));
		XDraw_DrawRoundRectF(hDraw, &shadowRc, (float)ctx->cornerRadius, (float)ctx->cornerRadius);
	}
	XDraw_SetLineWidth(hDraw, 1);
...
_XCal_Ctx* _XCal_CreateWindow(_XCal_Mode mode, BOOL doubleMonth, HWINDOW hParent,
	xcalendar_theme_ theme, int cornerRadius)
{
	...
	int winW = contentW + kShadowMargin * 2;
	int winH = contentH + kShadowMargin * 2;
	...
	if (mode == _XCal_Mode_Single){
		const int left = 24;
		const int top = 18;
		ctx->hDialogTitle = _XCal_CreateText(ctx, left, top, 160, 28, L"选择日期",
			ctx->colors.text, textAlignFlag_left | textAlignFlag_vcenter);
		ctx->hBtnPrevYear = _XCal_CreateCmdButton(ctx, left, 58, 28, 28, L"<<", _XCal_Cmd_PrevYear);
		ctx->hBtnPrevMonth = _XCal_CreateCmdButton(ctx, left + 34, 58, 28, 28, L"<", _XCal_Cmd_PrevMonth);
		ctx->hBtnNextMonth = _XCal_CreateCmdButton(ctx, 240, 58, 28, 28, L">", _XCal_Cmd_NextMonth);
		ctx->hBtnNextYear = _XCal_CreateCmdButton(ctx, 274, 58, 28, 28, L">>", _XCal_Cmd_NextYear);
		...
		ctx->hTitleLeft = _XCal_CreateText(ctx, 110, 58, 120, 28, L"", ctx->colors.title);
		_XCal_BuildGrid(ctx, ctx->hCellsLeft, left, 105, FALSE);
		ctx->hActionWrap = _XCal_CreateActionLayout(ctx, left + 2, 392, 276, 32);
		if (ctx->hActionWrap){
		...
		const int left = 30;
		const int top = 36;
		const int navSize = 28;
		const int space = 6;
		const int rightStart = ctx->doubleMonth ? 340 : 30;

		ctx->hBtnPrevYear = _XCal_CreateCmdButton(ctx, left, top, navSize, navSize, L"<<", _XCal_Cmd_PrevYear);
		ctx->hBtnPrevMonth = _XCal_CreateCmdButton(ctx, left + navSize + space, top, navSize, navSize, L"<", _XCal_Cmd_PrevMonth);
		ctx->hBtnNextMonth = _XCal_CreateCmdButton(ctx, rightStart + 216, top, navSize, navSize, L">", _XCal_Cmd_NextMonth);
		ctx->hBtnNextYear = _XCal_CreateCmdButton(ctx, rightStart + 216 + navSize + space, top, navSize, navSize, L">>", _XCal_Cmd_NextYear);
		_XCal_SetButtonIcon(ctx->hBtnPrevYear, ctx->hImgLastYear, ctx->hImgLastYearDis);
		_XCal_SetButtonIcon(ctx->hBtnPrevMonth, ctx->hImgLastMonth, ctx->hImgLastMonthDis);
		_XCal_SetButtonIcon(ctx->hBtnNextMonth, ctx->hImgNextMonth, ctx->hImgNextMonthDis);
		_XCal_SetButtonIcon(ctx->hBtnNextYear, ctx->hImgNextYear, ctx->hImgNextYearDis);
		...
		if (ctx->doubleMonth){
			ctx->hTitleLeft = _XCal_CreateText(ctx, 120, top, 120, navSize, L"", ctx->colors.title);
			ctx->hTitleRight = _XCal_CreateText(ctx, 430, top, 120, navSize, L"", ctx->colors.title);
			_XCal_BuildGrid(ctx, ctx->hCellsLeft, left, 80, FALSE);
			_XCal_BuildGrid(ctx, ctx->hCellsRight, rightStart, 80, TRUE);
		} else {
			ctx->hTitleLeft = _XCal_CreateText(ctx, 116, top, 120, navSize, L"", ctx->colors.title);
			_XCal_BuildGrid(ctx, ctx->hCellsLeft, left, 80, FALSE);
		}

		int botY = 356;
		ctx->hBtnToday = _XCal_CreateCmdButton(ctx, left, botY, 64, 28, L"今天", _XCal_Cmd_Today, FALSE, TRUE);
		ctx->hBtnLast7 = _XCal_CreateCmdButton(ctx, left + 70, botY, 64, 28, L"近7天", _XCal_Cmd_Last7, FALSE, TRUE);
		ctx->hBtnLast15 = _XCal_CreateCmdButton(ctx, left + 140, botY, 64, 28, L"近15天", _XCal_Cmd_Last15, FALSE, TRUE);
		ctx->hBtnLast30 = _XCal_CreateCmdButton(ctx, left + 210, botY, 64, 28, L"近30天", _XCal_Cmd_Last30, FALSE, TRUE);
		ctx->hRangeText = _XCal_CreateText(ctx, ctx->doubleMonth ? 400 : 30, botY + (ctx->doubleMonth ? 0 : 32),
			ctx->doubleMonth ? 220 : 280, 28, L"", ctx->colors.rangeText,
			ctx->doubleMonth ? (textAlignFlag_left | textAlignFlag_vcenter) : (textAlignFlag_center | textAlignFlag_vcenter), TRUE);

		int editY = 410;
		int editW = ctx->doubleMonth ? 168 : 158;
		int editH = 32;
		ctx->hEditStart = _XCal_CreateEdit(ctx, left, editY, editW, editH, TRUE,
			&ctx->hEditWrapStart, &ctx->hSpinWrapStart, &ctx->hSpinStartUp, &ctx->hSpinStartDown);
		int edit2X = ctx->doubleMonth ? 230 : 30;
		int actionX = ctx->doubleMonth ? 450 : 206;
		if (ctx->doubleMonth){
			ctx->hEditEnd = _XCal_CreateEdit(ctx, edit2X, editY, editW, editH, FALSE,
				&ctx->hEditWrapEnd, &ctx->hSpinWrapEnd, &ctx->hSpinEndUp, &ctx->hSpinEndDown);
		} else {
			ctx->hEditEnd = _XCal_CreateEdit(ctx, left, editY + 42, editW, editH, FALSE,
				&ctx->hEditWrapEnd, &ctx->hSpinWrapEnd, &ctx->hSpinEndUp, &ctx->hSpinEndDown);
			actionX = 206;
			editY += 84;
		}
		ctx->hBtnCancel = _XCal_CreateCmdButton(ctx, actionX, editY, 70, editH, L"取消", _XCal_Cmd_Cancel, FALSE, TRUE);
		ctx->hBtnConfirm = _XCal_CreateCmdButton(ctx, actionX + 82, editY, 80, editH, L"确认", _XCal_Cmd_Confirm, TRUE);
	}
...
BOOL _XCal_Show(_XCal_Mode mode, BOOL doubleMonth, HWINDOW hParent,
	...
	BOOL bLimitMaxDate, const xcalendar_datetime_* pMaxDate, xcalendar_theme_ theme,
	int cornerRadius)
{
...
BOOL CXCalendarCard::PopupSingle(HWINDOW hParent, xcalendar_datetime_* pDate,
	BOOL bLimitMaxDate, xcalendar_theme_ theme, const xcalendar_datetime_* pMaxDate,
	int nCornerRadius)
{
