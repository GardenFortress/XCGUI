...
//                       / XE_DESTROY 4 个 C1 风格事件.
//       - 重复 AddEleTip: 仅更新文本, 事件已挂不重复挂.
//       - 事件回调先从注册表查询; 若已 DelEleTip 则什么也不做 (XCGUI 暂无 C1
//         版本的 RemoveEventC1, 留挂着安全, 元素 destroy 时 XCGUI 自动清).
//
...
BOOL CXTooltip::DelEleTip(HELE hEle)
{
	...
	XEle_KillXCTimer(hEle, kTimerId_ShowDelay);
	g.registry.erase(it);
	// 不调 XEle_RemoveEventC (XCGUI 暂无 C1 版本的 Remove), 留挂着 — 回调里
	// 会先 find registry, 找不到直接返 0, 安全无副作用. 元素 destroy 时 XCGUI
	// 自动清.
	return TRUE;
}
