...
public:
	_CXEditDW_ColorEffect(COLORREF c) : m_ref(1), m_color(c){}
...
class _CXEditDW_TextRenderer : public IDWriteTextRenderer{
	...
	ID2D1Brush* GetOrCreateColorBrush(COLORREF col) const{
		...
		if (!m_rt) return NULL;
		// COLORREF 是 XCGUI 风格的 0xAARRGGBB. 转 D2D 颜色 (与全局 RgbaToD2D 等价的内联实现).
		BYTE a = (BYTE)((col >> 24) & 0xFF);
		BYTE r = (BYTE)((col >> 16) & 0xFF);
		BYTE g = (BYTE)((col >>  8) & 0xFF);
		BYTE b = (BYTE)( col        & 0xFF);
		if (a == 0) a = 0xFF; // alpha=0 视为不透明 (与 XCGUI 0xRRGGBB 输入兼容)
		D2D1_COLOR_F c = D2D1::ColorF(r / 255.0f, g / 255.0f, b / 255.0f, a / 255.0f);
...
public:
	_CXEditDW_TextRenderer(ID2D1RenderTarget* rt, ID2D1Brush* defaultBrush, IDWriteFactory2* pDW2)
		: m_ref(1), m_rt(rt), m_defaultBrush(defaultBrush), m_pDW2(pDW2) {}
...
// COLORREF: XCGUI 风格的 0xAARRGGBB / 0xRRGGBB -> Win32 GDI 的 0x00BBGGRR.
// 取低 24 位重排 RGB->BGR. alpha 被丢弃 (GDI 文本不支持).
static COLORREF _ToGdiColor(COLORREF xcCol){
	BYTE r = (BYTE)((xcCol >> 16) & 0xFF);
	BYTE g = (BYTE)((xcCol >>  8) & 0xFF);
	BYTE b = (BYTE)( xcCol        & 0xFF);
	return RGB(r, g, b);
}
...
public:
	_CXEditDW_GdiRenderer(IDWriteBitmapRenderTarget* bmpRT, IDWriteRenderingParams* params, COLORREF defGdiColor)
...
void CXEditDW::SetText(const wchar_t* pString){
	...
	ReleaseAllStyleResources();
	std::wstring s = pString ? pString : L"";
	InsertCharsRaw(0, s.c_str(), (int)s.size(), -1);
...
void CXEditDW::SetTextInt(int nValue){
	// 用 std::to_wstring 而不是 swprintf_s, 避免 <stdio.h> 在 PCH 里没拉进来时编译失败.
	SetText(std::to_wstring(nValue).c_str());
}
...
BOOL CXEditDW::ClipboardPaste(){
	...
	while (i < len){
		...
		if (sid != -1 && IsStyleIdValid(sid)){
			SetCharStyle(insertPos + i, j - i, sid);
		}
		i = j;
	}
...
void CXEditDW::SetCharStyle(int pos, int len, int iStyle){
	int n = (int)m_text.size();
	if (pos < 0) pos = 0;
	if (pos > n) pos = n;
		...
		if (inlineObj && IsStyleIdValid(curStyle)){
			...
			if (s.type == 0 /*font*/){
				if (s.hFontImageObj){
					...
					XFont_GetFontInfo((HFONTX)s.hFontImageObj, &fi);
					if (fi.name[0]){
						p.pLayout->SetFontFamilyName(fi.name, range);
					}
					if (fi.nSize > 0){
				...
				if (s.bColor){
					_CXEditDW_ColorEffect* eff = new _CXEditDW_ColorEffect(s.color);
...
int CXEditDW::OnPaintImpl(HELE /*hEle*/, HDRAW hDraw, BOOL* /*pbHandled*/){
	...
	if (m_text.empty() && !m_hint.empty() && m_pTextFormat){
		...
		if (pHintBrush){
			...
			rt->DrawText(
				...
				rcHint,
				pHintBrush,
				GetColorFontDrawOptions(rt)); // Win7 D2D1.0 无 ENABLE_COLOR_FONT, 这里返 NONE
			pHintBrush->Release();
		}
	}
	...
	if (!m_paragraphs.empty()){
		const bool slowPath = HasAnyColoredText();
		...
		if (pTextBrush){
			...
			if (slowPath){
				...
				pRenderer = new _CXEditDW_TextRenderer(rt, pTextBrush, pDW2);
			}
			for (size_t pi = 0; pi < m_paragraphs.size(); ++pi){
				_XEditDW_Para& p = m_paragraphs[pi];
				if (!p.pLayout) continue;
			...
			if ((hr == S_OK || hr == E_NOT_SUFFICIENT_BUFFER) && actualCount > 0){
				...
				if (SUCCEEDED(hr)){
					for (UINT32 k = 0; k < actualCount; ++k){
						const DWRITE_HIT_TEST_METRICS& mm = metrics[k];
						rt->FillRectangle(
...
void CXEditDW::OnPaintGdi(HDC hdcTarget, HDRAW hDraw){
	...
	if (m_text.empty() && !m_hint.empty() && m_pTextFormat){
		...
		if (SUCCEEDED(hr) && pHintLayout){
			_CXEditDW_GdiRenderer* pR = new _CXEditDW_GdiRenderer(
				bmpRT, m_pParams, _ToGdiColor(m_hintColor));
	...
	if (!m_paragraphs.empty()){
		_CXEditDW_GdiRenderer* pR = new _CXEditDW_GdiRenderer(
			bmpRT, m_pParams, _ToGdiColor(m_textColor));
		float viewTop    = scrollY;
