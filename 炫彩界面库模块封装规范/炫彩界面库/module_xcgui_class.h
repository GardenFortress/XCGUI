...
//@别名  置鼠标悬停触发时间()
	void SetMouseHoverTime(int time) { XWnd_SetMouseHoverTime(m_hWindow, time); }
...
//@返回  如果窗口位置改变返回真, 否则相反
//@别名  调整到屏幕内()
	BOOL AdjustInScreen(HWINDOW hWindow, int nBorderSpace = 0, BOOL bCoverTaskBar = FALSE) { return XWnd_AdjustInScreen(m_hWindow, nBorderSpace, bCoverTaskBar); }
...
//@返回  成功返回TRUE,否则相反
//@别名  置工具提示持续时间()
	BOOL SetToolTipDuration(int nDuration){ return XEle_SetToolTipDuration(m_hEle, nDuration); }
...
//@参数  bEnable 是否启用
//@别名  启用URL下划线()
	void EnableUrlUnderline(BOOL bEnable){XEdit_EnableUrlUnderline(m_hEle,bEnable);}
...
//@返回 返回当前位置
//@别名 取当前位置()
	int GetCurPos(){return XSBar_GetCurPos(m_hEle);}
...
//@参数  nWidth 宽度
//@别名  置左侧宽度()
	void SetLeftWidth(int nWidth){ XMenu_SetLeftWidth(m_hMenu,nWidth); }
...
//@参数  pRect  矩形坐标
//@别名  转换坐标()
	void ConvRect(RECT* pRect){ XDraw_ConvRect(m_hDraw, pRect);}

//@备注  从当前UI对象客户区坐标转换到画布对齐坐标, 自动适应DPI, 便于用户使用第三方绘图接口
//@参数  x  X坐标
//@参数  y  Y坐标
//@别名  转换坐标XY()
	void ConvXY(int x, int y){ XDraw_ConvXY(m_hDraw, x, y);}
